...
<script type="text/javascript" src="<%=webPath%>/sams/ord/approve/OrdApproveOfTimeEdit.js?v=<%=Math.random()%>"></script>
...
<div id="searchTool">
<!-- 	<input type="hidden" id="apparatusId" value="1"/> -->
	
	<table cellpadding="0" cellspacing="0">
		<tr>
			<td class="searchTool_name w110">仪器名称：</td>
			<td colspan="5" class="fr">		
				<select id="apparatusId_time" onchange="changeApparatusId()" class="w1000">
					<ec:options items="getApparatusMap"/>
				</select>
			</td>
		</tr>
	</table>
...
<input type="hidden" id="applyTime" value="${appString }"/><input type="hidden" id="timeFlag"/><input type="hidden" id="resultFlag" value="true"/>
<input type="hidden" id="appAllTime"/>

<c:forEach items="${apparList}" var="item" varStatus="status" begin="0" end="1">
	<input type="hidden" id="isSequencer" name="isSequencer" value="${item.isSequencer }"/>
	<input type="hidden" id="maxForecast" name="maxForecast" value="${item.maxForecast }"/>
	<input type="hidden" id="isManyIntoOut" name="isManyIntoOut" value="${item.isManyIntoOut }"/>
	<input type="hidden" id="maxSamples" name="maxSamples" value="${item.maxSamples }"/>
...
<div class="container">
						...
						<table style="margin-top:-40px" width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td colspan="6" class="fr">
									<input id="btnClose" type="button" value="添加仪器" class="btn btn-default fr" onclick="doOpenAddAppar()" />
								</td>
							</tr>
						</table>
			...
			<div class="popupBox fl" style="width:850px;" id="timeInfo">
				...
				<div>
					<table style="margin-top:-40px" width="100%" border="0" cellspacing="0" cellpadding="0" id="timeTable1">
						<tr>
							<td colspan="6" class="fr">
								<input id="btnClose" type="button"  value="预约时间" class="btn btn-default fr" onclick="timeControl()" />
							</td>
						</tr>
					</table>
					<div id="timeDiv"></div>
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="popupBox fl" style="width:850px;" id="sampleInfo">
					...
					<div>
						<table  border="0" style="width:100%">
							...
							<tr>
								...
								<td class="need w10"></td>
								<td><input type="text" class="w140 Wdate" id="ordSendsampleTime" name="ordSendsampleTime" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',errDealMode:0,readOnly:true});" value="${approveApply.ordSendsampleTime }"/></td>
								<td class="formName w110">完成时间</td>
								<td class="need w10"></td>
								<td><input type="text" class="w140 Wdate" id="ordFinishtime" name="ordFinishtime" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',errDealMode:0,readOnly:true});" value="${approveApply.ordFinishtime }"/></td>
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="ilead-table-box fl" style="width:850px;" id="itemInfo">
				<h3>检测项目及标准 </h3>
				<ec:table items="resultItemList" var="resultPara" retrieveRowsCallback="limit" tableId="ectable" 
					...
					<ec:row recordKey="${resultPara.ordItemstandardId}">
					    ...
					    <ec:column style="text-align:center;" width="10%" property="_2" title="操作" sortable="false" editable="false" viewsAllowed="html">
							<a style="color:black" href="javascript:return void(0)" onclick="deleItemStan(this,'edit','${resultPara.ordItemstandardId}')"><!-- <span class="glyphicon glyphicon-remove"></span> -->删除</a>
						</ec:column>
					</ec:row>
				</ec:table>
				...
				<textarea id="add_template" rows="" cols="" style="display:none">
					...
					<tpsp />
						<a style="color:black" href="javascript:" onclick="deleItemStan(this,'add','${resultPara.ordItemstandardId}')"><!-- <span class="glyphicon glyphicon-remove"></span> -->删除</a>
					<tpsp />				  	
				</textarea>
			</div>	
		</div>
	...
	<div class="row" id = "consumListDiv">
		<div class="col-xs-12">
			<div class="ilead-table-box fl"  style="width:850px;" id="consumablesInfo">
			<h3>耗材信息 </h3>
				<ec:table items="consumList" var="resultcon" retrieveRowsCallback="limit" tableId="ectableConsu" 
					...
					<ec:row recordKey="${resultcon.materialCostId}">		
						...
						<ec:column width='20%' property="consumablesName"  title="耗材名称" ellipsis="true" editTemplate="ecs_t_consumablesName">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<td style="width: 150px">
						<input  type="text" style="width: 140px" name="ordActualFee" id="ordActualFee" value="${approveApply.ordActualFee }"/>
					...
					<td style="width: 100px" align="left">
						<input id="btnClose" type="button"  class="btn btn-default" value="明细" onclick="viewPriceDetail()" />
					</td>
				</tr>
			</table>
		</div>
...
<div class="row">
		<div class="col-xs-12">
			<div class="popupBox fl  m-b-60" style="width:850px;" id="otherInfo">
				<form id="otherInfoForm" name="otherInfoForm">
					...
					<div>
						<table  border="0" style="width:100%;" >
							<tr>
								...
								<td class="need w10"></td>
								<td>
									<textarea rows="3" cols="110"  id="remark" name="remark" >${approveApply.remark }</textarea>
								</td>
							</tr>
						</table>
					</div>
					<c:if test="${(actionType != 'edit' &&  actionType != 'copy') || approveApply.ordState =='02' || approveApply.ordState =='03' || approveApply.ordState =='07'}">
						...
						<table  border="0" style="width:100%;" >
							<tr>
								...
								<td class="need w10"></td>
								<td>
									<textarea rows="3" cols="110" id="ordResponse" name="ordResponse" readonly="readonly" >${approveApply.ordResponse}</textarea>
								</td>
							</tr>
						</table>
					</c:if>
					<c:if test="${actionType != 'copy' }">
							<table  border="0" style="width:100%" >
								<tr>
									<td class="formName" style="width:600px">打印明细
