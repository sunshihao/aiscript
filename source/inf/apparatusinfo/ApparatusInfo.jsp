...
<body>
<div class="wrapper">
    <form class="z-page-search" method="post" action="apparatusInfoAction.do?method=doInit" id="apparatusInfoForm"
          name="apparatusInfoForm">
        <input id="belongCenterHidden" type="hidden"
               value="<%=request.getParameter("belongCenter")==null?"":request.getParameter("belongCenter") %>"/>
        <input id="belongUnitHidden" type="hidden"
               value="<%=request.getParameter("belongUnit")==null?"":request.getParameter("belongUnit") %>"/>
        <input id="belongGroupHidden" type="hidden"
               value="<%=request.getParameter("belongGroup")==null?"":request.getParameter("belongGroup") %>"/>
        <input id="sampleTypeHidden" type="hidden"
               value="<%=request.getParameter("sampleType")==null?"":request.getParameter("sampleType") %>"/>
        <input id="elementNameHidden" type="hidden"
               value="<%=request.getParameter("elementName")==null?"":request.getParameter("elementName") %>"/>
        <input id="elementId" type="hidden"
               value="<%=request.getParameter("elementId")==null?"":request.getParameter("elementId") %>"/>
        <input id="isSwingCardHidden" type="hidden"
               value="<%=request.getParameter("isSwingCard")==null?"":request.getParameter("isSwingCard") %>"/>
        <div class="searchTool">
            <div class="z-page-search-form">
                <table class="z-page-search-componets">
                    ...
                    <tr id="hiddenTr2">
                        <td>
                            <span class="name">仪器中类:</span>
                            <select name="categoryMiddle" id="categoryMiddle" class="w140"
                                    onchange="changeCategoryMiddle()">
                                <option value="">请选择</option>
                            </select>
                        </td>
                        <td>
                            <span class="name">仪器小类:</span>
                ...
                <div class="z-page-search-fold">&nbsp;
                    ...
                    <input type="reset" class="btn_style  z-btn-primary" value="重置" onclick="doReset1()"/>
                    <%--                    <input type="button" class="btn_style" value="查询" onclick="doQueryInfo()"/>--%>
                    <%--                    <input type="reset" class="btn_style" value="重置" onclick="doReset1()"/>--%>
                    <%--                    <input type="button" class="btn_style" value="返回" onclick="doBack()"/>--%>
                    <a href="javascript:" onclick="getMoreCondition(this)">更多</a>
                </div>
            </div>
        </div>
    </form>
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-xs-6 text-left">
                            仪器名称与型号：<a href="javascript:" onclick="doView(this)"><span id="apparatusId"
                                                                                              style="display:none"></span><span
                                id="apparatusName"></span><span id="apparatusModel"></span></a>
                        </div>
                        <div class="col-xs-3">
